...
class Model:
    ...
    def execute(self):
        ...
        # list adjective pairs in order
        apfile = open("adjrank.txt", "w+")
        adjrank = Counter()
        for i, lword in enumerate(left):
            for j, rword in enumerate(right):
                adjrank[(adjectives[i],adjectives[j])] = self.dot_product(lword, rword)
        for cluster_combination in adjrank.most_common(self.top_count * self.top_count):
            apfile.write("\n{}".format(cluster_combination))
        ...
        for cluster_combination in dot_products.most_common(self.num_best_clusters):
            ...
            if len(left_clusters_to_words[left_cluster]) > 1 and len(right_clusters_to_words[right_cluster]) > 1:
